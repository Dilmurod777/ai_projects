...
from keras.layers import Lambda, Dense, Flatten, Reshape, Conv2D, Dropout, MaxPooling2D
import numpy as np
import logging
import copy
import random
...
# methods to create model definition
	...
	for layer in layers:
		...
		if layer_type == 'DENSE':
			...
			if layer_type == 'CONV2D':
				...
				last_shape = (np.ceil(last_shape[0] / stride[0]), np.ceil(last_shape[1] / stride[1]), size)
			elif layer_type == 'MAXPOOL2D':
				weights = 0
				last_shape = (np.ceil(last_shape[0] / stride[0]), np.ceil(last_shape[1] / stride[1]), last_shape[2])
...
def add_layer(model, params, prior_type):
	...
	if layer_type == 'DENSE':
		...
		model.add(Conv2D(size, kernel_size, activation=act, strides=stride, padding='same'))
	elif layer_type == 'MAXPOOL2D':
		kernel_size = params['kernel_size']
		stride = params['stride']
		model.add(MaxPooling2D(pool_size=kernel_size, strides=stride, padding='same'))
...
def create_and_evaluate_model(model_definition):
	...
	history = model.fit(train_images, train_labels,
											...
											epochs=EVAL_EPOCHS, batch_size=64, verbose=2, shuffle=False)
	acc = history.history['val_accuracy'][-1]
	print(f'Size: {weight_count}')
...
for i in range(CANDIDATE_EVALUATIONS):
	...
	print(f'Random search, best accuracy: {val_accuracy:5.2f}')
		...
		if np.random.randint(0, 2) == 1:
			...
			if delta == -1:
				...
				if layer_num == 0 and first_dense == 1:
					layer_num += 1  # require >= non-dense layer
...
for i in range(CANDIDATE_EVALUATIONS):
	...
	while not valid_model:
		...
		if acc > val_accuracy:
			...
			model_definition = old_model_definition
	print(f'Hill climbing, best accuracy: {val_accuracy:5.2f}')
...
history = model.fit(train_images, train_labels,
										validation_data=(test_images, test_labels),
										epochs=FINAL_EPOCHS, batch_size=64, verbose=2, shuffle=True)
