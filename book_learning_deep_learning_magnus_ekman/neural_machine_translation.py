...
# create a sample of the test set that will be inspected in detail
...
# we do not use the last layer output, only the state
...
for i in range(EPOCHS):
	...
	history = training_model.fit([train_src_input_data, train_dest_input_data],
	...
	for (test_input, test_target) in zip(sample_input_data, sample_target_data):
		# run a simple sentence through encoder model
		x = np.reshape(test_input, (1, -1))
		last_states = enc_model.predict(x, verbose=0)
		# provide resulting state and START_INDEX as input to decoder model
